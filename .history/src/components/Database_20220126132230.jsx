...
const Database = (data0) => {
  ...
  console.log(data0.data0.region)
        ...
        <form className="d-flex flex-column m-3">
          ...
          {regions.map((item) => (
            <div>
              ...
              <label className="mx-1" for="region" onClick={() => console.log(this.item.region)}>
                {item}
                {data0.data0.filter((item) => data0.data0.includes(item.region)).length}
              </label>
            </div>
          ))}
        </form>
