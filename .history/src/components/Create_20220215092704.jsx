...
const Create = (childnum) => {
  ...
  const writeData = () => {
    if (document.getElementById("name").value && document.getElementById("age").value && document.getElementById("color").value && document.getElementById("sex").value && document.getElementById("region").value && document.getElementById("city").value && document.getElementById("phone").value) {
      Firebase.database()
        .ref("/cats")
  ...
  const [url, setURL] = useState("https://i.postimg.cc/ncPdBCCp/4dadbbe6d6872bbeee3e69a46b70bdd8.jpg");
  ...
  return (
    <Container>
      ...
      <Card className="card-profile">
        ...
        <input  id="name" className="my-3" type="text" name="firstname" placeholder="Wpisz imię kota.." />
        ...
        <form className="d-flex flex-row" onSubmit={handleUpload}>
          <input type="file" onChange={handleChange} />
          <button disabled={!file}>wyślij zdjęcie</button>
        </form>

        <Card.Body>
          <form className="d-flex flex-column w-25" action="/action_page.php">
      ...
      <div className="success-box" onClick={() => (document.getElementsByClassName("success-box")[0].style.visibility = "hidden")}>
