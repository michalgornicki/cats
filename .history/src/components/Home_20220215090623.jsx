...
const Home = (props) => {
  const regions = ["Polska", "Dolnośląskie", "Kujawsko-pomorskie", "Lubelskie", "Lubuskie", "Łódzkie", "Małopolskie", "Mazowieckie", "Opolskie", "Podkarpackie", "Podlaskie", "Pomorskie", "Śląskie", "Świętokrzyskie", "Warmińsko-mazurskie", "Wielkopolskie", "Zachodniopomorskie"];
    ...
    if (data1.filter((e) => e.user === user.name).length < 1) {
      Firebase.database()
        ...
        .set({
          id: usernum + 1,
          user: user.name,
          favourites: props.catsData,
        });
    } else if (user) {
      Firebase.database().ref("/users").child(usernum).set({
        favourites: props.catsData,
      });
    }
  };
